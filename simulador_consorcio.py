...
if st.button("Simular"):
    ...
    with colA:
        ...
        if taxa_antecipacao > 0:
            if not df_parcelas.empty and df_parcelas.loc[0, 'Total (R$)'] != "Total":
                st.write(f"**1ª e 2ª parcelas:** A partir de {formatar_moeda(df_parcelas.loc[0, 'Total (R$)'])}")
                st.write(f"**Média das demais parcelas:** {formatar_moeda(df_parcelas.loc[2:len(df_parcelas)-2, 'Total (R$)'].mean())}")
                st.info("Parcelas 1 e 2 contém antecipação do pagamento. As demais parcelas têm incidência do índice de correção estimado.")
            else:
                st.write(f"**Média das parcelas:** {formatar_moeda(df_parcelas.loc[0:len(df_parcelas)-2, 'Total (R$)'].mean())}")
                st.info("As parcelas têm incidência do índice de correção estimado.")
